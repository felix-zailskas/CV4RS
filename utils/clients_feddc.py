...
def train_model_FedDC(model, data_loader, device, epochs, alpha, local_update_last, global_update_last, global_model_param, hist_i):
...
class GlobalClientFedDC:
    def __init__(
        ...
        self.weight_list = self.client_data_lengths / np.sum(self.client_data_lengths) * self.n_clients
        ...
        # dim = (#clients, #params)
        self.clnt_params_list = np.ones(self.n_clients).astype("float32").reshape(-1, 1) * self.init_par_list.reshape(1, -1)
        self.clnt_models = list(range(self.n_clients))
        # includes global model
        self.state_gradient_diffs = np.zeros((self.n_clients + 1, self.n_par)).astype("float32")
    ...
    def train(self, communication_rounds: int, epochs: int):
        ...
        self.avg_model.load_state_dict(copy.deepcopy(dict(self.init_model.named_parameters())), strict=False)
        ...
        self.cur_cld_model.load_state_dict(copy.deepcopy(dict(self.init_model.named_parameters())), strict=False)
    ...
    def communication_round(self, epochs: int):
        global_mdl = torch.tensor(self.cld_mdl_param, dtype=torch.float32, device=self.device)
        ...
        for clnt_i, clnt in enumerate(self.clients):
            ...
            model.load_state_dict(copy.deepcopy(dict(self.cur_cld_model.named_parameters())), strict=False)
            # make sure to collect the gradients
            for params in model.parameters():
                params.requires_grad = True
            local_update_last = self.state_gradient_diffs[clnt_i]
            # global model at last pos
            global_update_last = self.state_gradient_diffs[-1] / self.weight_list[clnt_i]
            alpha = self.alpha_coef / self.weight_list[clnt_i]
            hist_i = torch.tensor(self.parameter_drifts[clnt_i], dtype=torch.float32, device=self.device)
            # train client
            self.clnt_models[clnt_i] = train_model_FedDC(model, self.data_loaders[clnt_i], self.device, epochs, alpha, local_update_last, global_update_last, global_mdl, hist_i)
            
            curr_model_par = get_mdl_params([self.clnt_models[clnt_i]])
            delta_param_curr = curr_model_par - self.cld_mdl_param
            self.parameter_drifts[clnt_i] += delta_param_curr.reshape(delta_param_curr.shape[1],)
            n_minibatch = (np.ceil(self.client_data_lengths[clnt_i] / 128) * epochs).astype(
                np.int64
            )
            beta = 1 / n_minibatch / 0.001
            
            state_g = local_update_last - global_update_last + beta * (delta_param_curr)
            delta_g_cur = (state_g - self.state_gradient_diffs[clnt_i]) * self.weight_list[clnt_i]
            delta_g_sum += delta_g_cur.reshape(delta_g_cur.shape[1],)
            self.state_gradient_diffs[clnt_i] = state_g
            self.clnt_params_list[clnt_i] = curr_model_par.reshape(curr_model_par.shape[1],)

        avg_mdl_param = np.mean(self.clnt_params_list, axis=0)
        delta_g_cur = 1 / self.n_clients * delta_g_sum
        self.state_gradient_diffs[-1] += delta_g_cur.reshape(delta_g_cur.shape[1],)
        
        self.cld_mdl_param = avg_mdl_param + np.mean(self.parameter_drifts, axis=0)
        self.avg_model = set_client_from_params(model_func(**model_args), np.mean(self.clnt_params_list, axis=0), self.device)
        self.cur_cld_model = set_client_from_params(model_func(**model_args).to(self.device), np.mean(self.clnt_params_list, axis=0), self.device)
